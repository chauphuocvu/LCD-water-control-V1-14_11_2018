...
{    
		  /* Set PA7 to measure how long does it take to generate wave and update buffer*/
...
{
	//Prepare for next transfer after transmitting the buffer 0.
	fill_dma_buf(1);
	
	//Update the buffer. This function is in circular_buffer.c
	updateBuffer();
}
...
{
	//Prepare for next transfer after transmitting the buffer 1.
	fill_dma_buf(0);
	updateBuffer();
}
